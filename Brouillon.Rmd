---
title: "Using click through rates to assess political trust: The Trudeau Polimeter case "
author: 
...
*H1) Assuming that Polimeter users who consult promise pages from a specific issue category are comparable to CES users who consider this category as their most preferred issue, we expect to find stronger effect on political trust from Google Analytics when using the number of page views from a user and the duration of these views compared to the other types of data from Google Analytics.* 

The comparability of these 2 data sets will be further explained in the  "Data and Methodology" section.
...
Assuming that Polimeter users who consult promise pages from a specific issue category are comparable to CES users who consider this category as their most preferred issue, we contend that a page view on a promise should should lead to political trust.

*H2) Assuming that Polimeter users are comparable to sophisticated CES users who consult promise pages from a specific issue category are comparable to CES users who consider this category as their most preferred issue, we predict that CES users with a higher level of political sophiscation should receive greater influence for a broken or kept pledge on political trust since they are more inclined to use information seeking media such as CPETs.*

Details on this hypothesis will be further explained in the “Data and Methodology” section.
...
However, since the Polimeter's research group is at his first time using data from Google Analytics, they have been facing few obstacles regarding its usage. First of all, The Polimeter changed his website structure once which led to having two different time periods of available data for both Polimeter websites. In addition, There has been an major update to Google Analytics which has became Google Analytics 4. Theses changes caused having another division of websites, leaving us with three different websites which means three different time periods to clean and include and our analysis. For practical reasons, we decided to only use the most recent data of the Polimeter's website, which is on Google analytics 4. This choice of data was the most adapted for our research, with the others being more time consuming. We were left with Google Analytics data ranging from August 2021 to December 2023. The data types extracted from the Polimeter are *"Total views, Views by user, Durations and Events"*. The structure of this CPET leads us to agregate these by classifying them by issue categories. Indeed, the Polimetre has 12 issues groups such as *"Economy and Employability, Health and social services or Minorities"*. 

These categories are primordial in order to create the link in between the Polimeter's data and our other dataset, the 2021 Canadian electoral study (CES). This is because there is a question in the CES asking about the most preferred issue of the respondent. Assuming that a view on a specific category on the Polimeter is equal to a particular interest for this issue categore, we are able to apply those aggregated values by issue categories on every CES respondents that have the same most preferred issue. The objective is to answer the first hypothesis by verifying with linear regressions if theses aggregated values have an effect on CES respondants political trust and if some of them aren't manifesting correlations.

Speaking of which, three factorial analysis have been conducted on selected CES variables to figure out which ones could potentially be fused to create a degree of political trust for every respondant. Based on figure 1, all ten questions are above the arbitrary level of 0.3, hence are selected to create that new political trust variables. The second factorial analysis has been made to create a political sophistication variable, from which 3 out of 14 were under the treshold of 0.3. The third factorial analysis confirmed that the 11 remaining variables were linked together and could be added together to create such a variable. This political sophistication variable will be used to answer our second hypothesis. We also added a liberal vote variable since we assume that:

*H3) Assuming that Polimeter users are comparable to CES users who consult promise pages from a specific issue category are comparable to CES users who consider this category as their most preferred issue, we expect that CES users who have voted for the Liberal party will express more trust towards a liberal government, such as Trudeau's.*

# Résultats et Analyses

Since our theory mobilizes many concepts that leads to complex hypotheses containing many dependant variables to political trust, the linear regressions models result in 4 tables presenting lots of political trust coefficients.  we have 2 univariate tables, one for no political sophistication threshold and the other having a 0.65 threshold of sophistication. We also have 2 similar tables but for multivariate regressions. Let's break them down.

First, for the univariate regressions, we observe slight positive effects on political trust for some issues categories, such as Education, Environment, Families, Health & social services, and Identity & nationalism, while the others have slight negative effects. Others socio-demographic control variables seems to have slight effects on political trust, but nothing major, except for the education level which has a coefficient of 0.102 with 0.009 of statistical significance. The inclusion of a sophistication threshold increases theses coefficients by a little. When applied in multivariates regressions, the results are relativaly similars, whether it is with or without a sophistication level included.

The third hypothesis can be quickly adressed, Since we can observe a positive effect on the liberal vote variable on political trust. This effects becomes stronger when the sophistication threshold is applied. This makes our third hypothesis confirmed.

When it comes to our first hypothesis, the univariate tables shows that the only Google Analytics variables that have an effect on political trust are the mean of views by user. These effects become stronger when adding the sophistication threshold. Similars tendencies can be found in the multivariates tables. The duration and event variables did not show any results in any of the 4 tables, which rules them out of our analysis. Finalement, total views seems to have an inverted effect compared to views by user, but this effect is considerably weaker, almost worth not mentionning. Since durations have no effects on political trust, and page views by user are alligned with our mobilized theory, we can partially confirm our first hypothesis. 

Finally, the finding mentionned in the previous paragraphs shows that almost all variables have an increase in their coefficient when the political sophistication threshold is added. The most important exception is the mean of page views by users in the multivariate analysis. When exposed to a higher level of sophistication, this variable knows a decrease in it's positive effect on political trust. Because of this, Our second variable is partially confirmed.
